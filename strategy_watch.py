...
def get_json(url):
    req = urllib.request.Request(url, headers={"User-Agent": "fpl-strategy-watch/1.0"})
    with urllib.request.urlopen(req, timeout=30) as response:
        return json.load(response)


def get_text(url):
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 fpl-strategy-watch/1.0"})
    with urllib.request.urlopen(req, timeout=30) as response:
...
def fixture_schedule(fixtures, teams, gw):
    ...
    for f in fixtures:
        ...
        if event is None:
            unassigned.append({
                "home": teams.get(h, str(h)),
                "away": teams.get(a, str(a)),
    ...
    for event in range(max(1, gw + 1), 39):
        ...
        if blanks or doubles:
            upcoming.append({
                "gw": event,
                "blank_teams": sorted(blanks),
                "double_teams": sorted(doubles),
                "status": "confirmed_from_fpl_fixture_assignment",
            })
    return upcoming, unassigned


def scout_watch():
    keywords = re.compile(r"blank|double|postpon|reschedul|cup|chip strategy|gameweek", re.I)
    items, errors = [], []
    seen = set()
        ...
        try:
            ...
            for href, title in re.findall(r'<a[^>]+href=["\']([^"\']+)["\'][^>]*>(.*?)</a>', text, re.I | re.S):
                ...
                if key in seen:
                    continue
                seen.add(key)
                items.append({"title": clean[:220], "url": href, "source": source})
                if len(items) >= 20:
                    break
        except Exception as exc:
            errors.append({"source": source, "error": str(exc)[:200]})
    return {"items": items[:20], "errors": errors, "source_count": len(SCOUT_URLS), "status": "ok" if items else "no_items"}
...
def main():
    ...
    target_rivals = sorted(
        [x for x in managers if x["entry_id"] != MY_ENTRY_ID and (x.get("total_points") or 0) > (me.get("total_points") or 0)],
        key=lambda x: (x.get("total_points") or 0) - (me.get("total_points") or 0),
    )[:3]
    ...
    for m in managers:
        ...
        m["chip_edge_vs_me"] = {
            "my_extra_chips": sorted(my_remaining - rem),
            "their_extra_chips": sorted(rem - my_remaining),
            "net_remaining_delta": len(my_remaining) - len(rem),
        }
    ...
    for r in target_rivals:
        ...
        if edge["my_extra_chips"]:
            strategic_notes.append({
                ...
                "note": f"You retain {', '.join(edge['my_extra_chips'])} that this rival has already spent in the current half. Preserve flexibility rather than copying short-term moves blindly.",
            })
        if edge["their_extra_chips"]:
            strategic_notes.append({
                "manager": r["manager"],
                "team_name": r["team_name"],
                "note": f"This rival retains {', '.join(edge['their_extra_chips'])} that you have already spent. Expect a future chip-driven swing and favour squad flexibility.",
            })
    ...
    result = {
        ...
        "chip_rules": {
            "two_sets": True,
            "first_half": "GW1-GW19",
            "second_half": "GW20-GW38",
            "chips_per_half": ["Wildcard", "Free Hit", "Triple Captain", "Bench Boost"],
        },
        "me": me,
        "target_rivals": target_rivals,
        "managers": managers,
        "confirmed_blank_double_events": schedule_events,
    ...
    print(json.dumps({
        ...
        "scout_items": len(result["scout_watch"]["items"]),
    }))
