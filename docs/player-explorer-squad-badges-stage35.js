(()=>{
const BUILD='player-explorer-squad-badges-20260830-2020';
...
function apply(){const host=document.getElementById('gp26-rows'),d=window.FPLCoreData;if(!host||!d)return;const actualNames=new Set(currentRows(d).map(p=>norm(p.player||p.web_name)).filter(Boolean)),confirmedIn=confirmedIncomingNames(d),moves=window.FPLWorkingPlan?.moves(plan())||[],workingIn=new Set(moves.map(m=>norm(m.in?.player))),workingOut=new Set(moves.map(m=>norm(m.out?.player)));
 [...host.children].forEach(row=>{const strong=row.querySelector('strong');if(!strong)return;row.querySelectorAll('[data-squad-badge]').forEach(x=>x.remove());let raw=strong.textContent||'';const hadSquad=/\s·\sSQUAD/i.test(raw),hadIn=/\s·\sYOUR IN/i.test(raw),hadOut=/\s·\sYOUR OUT/i.test(raw);raw=raw.replace(/\s·\sSQUAD/gi,'').replace(/\s·\sYOUR IN/gi,'').replace(/\s·\sYOUR OUT/gi,'');const name=norm(raw.replace(/^\d+\.\s*/,'').trim());const isCurrent=actualNames.has(name);strong.textContent=raw;row.dataset.squadOwned=isCurrent?'1':'0';row.style.background=isCurrent?'#1b2d4b':'#142139';row.style.borderColor=isCurrent?'#4778ad':'#243451';row.style.boxShadow=isCurrent?'inset 0 0 0 1px rgba(96,165,250,.48)':'';const add=(el)=>{el.dataset.squadBadge='1';strong.appendChild(el)};
 if(confirmedIn.has(name)){add(badge('↗','Committed transfer in · current squad','#86efac','#123326','#34d399'));}
 else if(workingIn.has(name)&&!isCurrent){add(badge('↗','Working transfer in','#fcd34d','#33280f','#fbbf24','','dashed'));}
 else if(isCurrent||hadSquad){add(badge('✓','Current squad','#bfdbfe','#17345a','#60a5fa'));}
 if(workingOut.has(name)||hadOut)add(badge('↘','Working transfer out','#fda4af','#36151d','#fb7185'));
 if(!isCurrent&&!confirmedIn.has(name)&&!workingIn.has(name)&&hadIn)add(badge('↗','Working transfer in','#fcd34d','#33280f','#fbbf24','dashed'));
