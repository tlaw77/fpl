(()=>{
const BUILD='league-pack-positioning-20260829-0808';
...
function evaluate(d){
 ...
 let status='BALANCED',tone='#34d399',summary='Your XI has a usable mix of pack coverage and differentiated upside.';
 if(shields.length<3&&edges.length>=5){status='TOO OPEN';tone='#fbbf24';summary='Your XI is carrying more divergence than pack coverage. Reliability and one important shield should take priority over another differential.'}
 else if(shields.length<3){status='LIGHT COVERAGE';tone='#fbbf24';summary='Pack coverage is thin. Consider restoring one strong common starter if the football case is close.'}
 else if(edges.length<1){status='LOW LEVERAGE';tone='#60a5fa';summary='You are well covered but have little route to gain. One high-quality difference is enough; avoid forcing several.'}
 else if(edges.length>4){status='HIGH VARIANCE';tone='#fbbf24';summary='You already have plenty of routes to gain. Prefer dependable upgrades and preserve the strongest shared core.'}
...
function intelCard(host,e){
 const old=host.querySelector('[data-gap-rival]'); if(!old)return;
 old.innerHTML=`<div class="panel-head"><div><p class="eyebrow">LEAGUE POSITIONING</p><h3>${esc(e.status)} · squad vs pack</h3></div><div class="subtle">${e.rivalN} rivals</div></div><p class="subtle">${esc(e.summary)}</p><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:9px"><div style="border-left:4px solid #60a5fa;background:#142139;border-radius:11px;padding:9px"><strong>Pack shields</strong><div class="subtle" style="margin-top:5px">${esc(e.shields.slice(0,5).map(name).join(', ')||'No strong pack shields in the XI')}</div></div><div style="border-left:4px solid #34d399;background:#142139;border-radius:11px;padding:9px"><strong>Your useful edges</strong><div class="subtle" style="margin-top:5px">${esc(e.edges.slice(0,5).map(name).join(', ')||'No clear quality differential')}</div></div></div><div style="margin-top:8px;border-left:4px solid #fb7185;background:#142139;border-radius:11px;padding:9px"><strong>Pack threats you lack</strong><div class="subtle" style="margin-top:5px">${esc(e.threats.slice(0,5).map(x=>name(x.p)).join(', ')||'No major uncovered pack threat')}</div></div><div style="margin-top:9px;padding:10px 11px;border-radius:11px;background:#101a2d;border:1px solid ${e.tone}55;border-left:4px solid ${e.tone}"><div style="display:flex;justify-content:space-between;gap:8px;align-items:center"><strong>Does your squad fit the pack strategy?</strong><span style="font-size:8px;font-weight:900;color:${e.tone};border:1px solid ${e.tone}55;border-radius:999px;padding:4px 7px;white-space:nowrap">${esc(e.status)}</span></div><div class="subtle" style="margin-top:5px">${e.shields.length} strong shields · ${e.edges.length} quality edges · ${e.threats.length} major uncovered threats</div><div class="subtle" style="margin-top:5px"><b style="color:${e.tone}">Squad aim:</b> ${esc(e.summary)}</div>${e.nearTxt?`<div class="subtle" style="margin-top:5px">Nearest-rival context: ${esc(e.nearTxt)}. Individual-rival chasing stays secondary this early in the season.</div>`:''}</div>`;
 old.dataset.packPositioning='1';
}
...
function settle(){[120,480,1050].forEach(ms=>setTimeout(render,ms))}
