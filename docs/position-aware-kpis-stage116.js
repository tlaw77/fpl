(()=>{
const BUILD='position-aware-kpis-stage116-20260914-3';
...
if(isLeader){const lead=second?mine-n(second.total_points):0,secondLive=second?n(second.live_calculated_points??second.gw_points):0,gwDelta=live-secondLive,trend=gwDelta>0?'↑':gwDelta<0?'↓':'→';return {mode:'leader',html:[...common,kpi(lead===0?'Joint lead':'Lead',lead===0?'Level':`+${lead}`,second?.team_name||'No rival','lead'),kpi('GW vs #2',`${signed(gwDelta)} ${trend}`,gwDelta>0?'Lead growing':gwDelta<0?'Lead shrinking':'Holding level',gwDelta>0?'growing':gwDelta<0?'shrinking':'level'),kpi('Bank',`£${n(me.bank).toFixed(1)}m`,`TV £${n(me.team_value).toFixed(1)}m`)].join('')};}
