(()=>{
const BUILD='player-explorer-relative-colour-20260829-0724';
...
function toneByRank(value,values){const clean=values.filter(Number.isFinite).sort((a,b)=>a-b);if(!Number.isFinite(value)||clean.length<3)return NEUTRAL;const q1=clean[Math.floor((clean.length-1)*.33)],q2=clean[Math.floor((clean.length-1)*.67)];return value>=q2?GOOD:value>=q1?MID:BAD}
...
function metricCells(row){
 const labels=['Model','Fixtures','Minutes','Value','EO'];
 const found=[];
 for(const label of labels){
   const span=[...row.querySelectorAll('span')].find(s=>s.textContent.trim()===label);
...
function apply(){const host=document.getElementById('gp26-rows');if(!host)return;const rows=[...host.children].filter(x=>x.querySelector('strong'));applyWorkload(rows);
 const parsed=rows.map(row=>{const cs=metricCells(row);return{row,cs,vals:cs.map(c=>num(c.querySelector('b')?.textContent))}}).filter(x=>x.cs.length===5);
 if(!parsed.length)return;
 const cols=[0,1,2,3].map(i=>parsed.map(x=>x.vals[i]));
 parsed.forEach(x=>x.cs.forEach((c,i)=>{const v=x.vals[i],colour=i===4?eoTone(v):toneByRank(v,cols[i]);const b=c.querySelector('b'),bar=c.querySelector('i');if(b){b.style.setProperty('color',colour,'important');b.style.fontWeight='900'}if(bar){bar.style.setProperty('background',colour,'important');bar.style.opacity='1'}c.dataset.relativeTone=colour===GOOD?'positive':colour===MID?'middle':colour===BAD?'weak':colour===SHIELD?'shield':'neutral';c.title=i===4?(v<30?'Low ownership: leverage opportunity':v>=70?'High ownership: shield/coverage':'Mid ownership: neutral context'):(colour===GOOD?'Strong relative to the visible comparison set':colour===MID?'Middle of the visible comparison set':colour===BAD?'Weak relative to the visible comparison set':'Relative context unavailable') }));
 document.documentElement.dataset.playerExplorerRelativeColourBuild=BUILD;
}
function bindControls(){const host=document.getElementById('dc-pool-view');if(!host)return;['gp26-q','gp26-pos','gp26-sort'].forEach(id=>{const el=document.getElementById(id);if(!el||el.dataset.relativeColourBound)return;el.dataset.relativeColourBound='1';el.addEventListener(id==='gp26-q'?'input':'change',()=>{setTimeout(apply,20);setTimeout(apply,120)},{passive:true})})}
function refresh(){[80,250,650,1200].forEach(ms=>setTimeout(()=>{bindControls();apply()},ms))}
function bind(){document.querySelector('#decision-nav button[data-view="pool"]')?.addEventListener('click',refresh,{passive:true});window.addEventListener('fplSafePlanUpdated',refresh);window.addEventListener('fplCoreDataReady',refresh,{passive:true});if(document.querySelector('.dashboard-view.active')?.id==='view-pool')refresh()}
