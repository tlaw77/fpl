...
def get(url):
    req=urllib.request.Request(url,headers={'User-Agent':'fpl-schedule-load/1.2'})
...
def player_index(boot,teams):
    ...
    for p in boot.get('elements',[]):
        ...
        names={person_norm(p.get('web_name')),person_norm(p.get('second_name')),person_norm(f"{p.get('first_name','')} {p.get('second_name','')}")}
        names.discard('')
    ...
    for team_block in (summary.get('boxscore') or {}).get('players',[]) or []:
        ...
        for stat_block in team_block.get('statistics',[]) or []:
            ...
            for a in stat_block.get('athletes',[]) or []:
                ...
                pid=match_player(name,club,index)
                if not pid:continue
                stats=a.get('stats') or []
                mins=minute_value(stats[min_idx]) if min_idx is not None and min_idx<len(stats) else minute_value(a.get('minutes'))
                if mins is None:
                    # Some ESPN soccer feeds omit a MIN column but expose participation flags.
                    mins=90.0 if a.get('starter') is True else (0.0 if a.get('didNotPlay') else None)
                out.append({'player_id':pid,'name':name,'minutes':mins,'started':bool(a.get('starter'))})
    # keep the richest row per player if the feed repeats stat groups
    best={}
    for row in out:
...
def main():
    ...
    pindex=player_index(boot,teams)
    ...
    start=min([now-timedelta(days=8),*(dates or [now])])
    ...
    for slug,label in LEAGUES.items():
        ...
        for ev in data.get('events',[]):
            ...
            seen.add(key)
            event_meta[key]={'date':dt,'slug':slug,'label':label,'event':ev,'clubs':mapped}
            for club,home_away in mapped:
                rows.append({'club':club,'date':dt.isoformat(),'competition':label,'competition_slug':slug,'event_id':ev.get('id'),'name':ev.get('name') or ev.get('shortName') or '', 'home_away':home_away})

    player_rows={}
    # Player-level workload is only needed for recently completed non-PL matches.
    recent_cutoff=now-timedelta(days=8)
    for (event_id,slug),meta in event_meta.items():
        ...
        try:summary=get(f'{ESPN}/{slug}/summary?event={event_id}')
            ...
            failures.append({'league':slug,'event_id':event_id,'type':'summary','error':str(exc)[:180]});continue
        for club,home_away in meta['clubs']:
            for a in extract_player_minutes(summary,club,pindex):
                player_rows.setdefault(str(a['player_id']),[]).append({
                    'date':meta['date'].isoformat(),'competition':meta['label'],'competition_slug':slug,
                    'event_id':event_id,'name':meta['event'].get('name') or meta['event'].get('shortName') or '',
                    'home_away':home_away,'minutes':a.get('minutes'),'started':a.get('started'),
    ...
    OUT.write_text(json.dumps({
        ...
        'source':'ESPN public scoreboard + match summaries','coverage':list(LEAGUES.values()),'range_start':start.isoformat(),'range_end':end.isoformat(),
        'clubs':by_club,'players':player_rows,'player_minutes_lookback_days':8,'failures':failures
